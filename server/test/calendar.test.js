    ...
    describe('# GET /api/calendar/:datetime', () => {
        it('should return a month\'s date', done => {
            ...
            agent
                ...
                .expect(200)
                .expect(res => {
                    expect(res.body.length).to.equal(42);
                    expect(res.body[0].date).to.equal(new Date('2016-8-28').getTime());
                    expect(res.body[41].date).to.equal(new Date('2016-10-8').getTime());
                    done();
                })
                .catch(done);
        });
